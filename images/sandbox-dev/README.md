...
The image intentionally contains no privileged settings, host mounts, Docker socket, or AWS credentials. Local Compose wiring is in `infrastructure/docker/compose.yaml`; use `scripts/smoke-compose.sh` to verify the control-plane and runtime network contract.
