...
const snapshotBucket = read('infrastructure/terraform/modules/s3/main.tf');
...
assert.match(snapshotBucket, /block_public_acls\s*=\s*true/);
assert.match(snapshotBucket, /block_public_policy\s*=\s*true/);
assert.match(snapshotBucket, /ignore_public_acls\s*=\s*true/);
assert.match(snapshotBucket, /restrict_public_buckets\s*=\s*true/);
assert.match(snapshotBucket, /object_ownership\s*=\s*"BucketOwnerEnforced"/);
assert.match(snapshotBucket, /sse_algorithm\s*=\s*"AES256"/);
